...
const formatGroupValue = (value: string | number) => value || "Bo'sh";
...
const getGroupTitle = (group: GroupedData) =>
  Object.entries(group._id)
    .map(([key, value]) => {
      ...
      return `${option?.label || key}: ${formatGroupValue(value)}`;
    })
    .join(" | ");
  ...
  const exportExcel = async () => {
    try {
      const response = await axios.post(
        `${baseUrl}/printing/painting/export`,
        { ids: selectedIds },
        {
          responseType: "blob",
          headers: {
      ...
      link.href = url;
      link.setAttribute("download", "Bo'yash.xlsx");
