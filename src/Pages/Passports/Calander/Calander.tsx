...
const rowClass =
  "grid grid-cols-[30px_150px_150px_250px_150px_150px_150px_150px_150px_250px_minmax(200px,1fr)_150px_150px_50px] w-full gap-[10px] p-lg text-primary border-b border-primary items-center min-w-fit";

const headerClass =
  "grid grid-cols-[30px_150px_150px_250px_150px_150px_150px_150px_150px_250px_minmax(200px,1fr)_150px_150px_50px] w-full gap-[10px] p-lg bg-secondary text-primary rounded-t-[8px] border-b border-primary min-w-fit";
...
const groupOptions = [
  ...
  { label: "Design", value: "prints.design.article" },
];
...
const getGroupPrinted = (group: GroupedData) =>
  ...
  group.items.reduce((sum, item) => {
    const printsTotal = getPrintsArray(item.prints).reduce(
      (innerSum, print) => innerSum + Number(print.printed || 0),
      0
    );

    return sum + printsTotal;
  }, 0);

const getGroupFinished = (group: GroupedData) =>
  group.totalFinished ??
  group.items.reduce((sum, item) => sum + Number(item.finished || 0), 0);

const getGroupWidth = (group: GroupedData) =>
  group.totalWidth ??
  group.items.reduce((sum, item) => sum + Number(item.width || 0), 0);
...
export const Calander = () => {
  ...
  const refresh = () =>
    axios(`${baseUrl}/printing/calander`)
  ...
  const groupData = () => {
    ...
    axios(`${baseUrl}/printing/calander/group`, {
      method: "GET",
  ...
  const handleEdit = (e: React.FormEvent) => {
    ...
    axios(`${baseUrl}/printing/calander/${editing?._id}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  const exportExcel = async () => {
    try {
      const response = await axios.post(
        `${baseUrl}/printing/calander/export`,
        { ids: selectedIds },
        {
          responseType: "blob",
          headers: {
      ...
      link.href = url;
      link.setAttribute("download", "calander.xlsx");
        ...
        {isGrouped &&
          groupedData.map((group, index) => (
            <div key={index} className="flex flex-col min-w-fit w-full">
              <div className="grid grid-cols-[1fr_160px_180px_180px_180px] gap-[10px] p-lg bg-secondary text-primary border-b border-primary min-w-fit w-full">
            ...
            <div className="flex items-center gap-lg justify-end">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  axios(`${baseUrl}/printing/calander/${deleting._id}`, {
                    method: "DELETE",
                    headers: {
                      Authorization: `Bearer ${localStorage.getItem("token")}`,
                    },
    ...
    <div className={rowClass}>
      ...
      {readOnly ? (
        ...
        <MultipleSelect
          ...
          onChange={(items) => update({ printIds: items })}
        />
      )}
      ...
      <p className="flex flex-wrap gap-[6px]">
        {readOnly
          ...
          : selectedPrints?.map((g) => <span key={g._id}>{g.order.name}</span>)}
      </p>
      ...
      <p className="flex flex-wrap gap-[6px]">
        {readOnly
          ? getPrintsArray(value.prints).map((g, index) => (
              <div key={g._id || index} className="flex items-center gap-[5px]">
                <img
                  src={`${import.meta.env.VITE_APP_API_URL}${
                    g.design?.imageUrl || ""
                  }`}
                  className="w-[50px] aspect-square object-cover"
                  alt={g.design?.article || ""}
                />
                <p>{g.design?.article}</p>
              </div>
            ))
          : selectedPrints?.map((g) => (
              <div key={g._id} className="flex items-center gap-[5px]">
                <img
                  src={`${import.meta.env.VITE_APP_API_URL}${
                    g.design?.imageUrl || ""
                  }`}
      ...
      <NumericFormat
        ...
        value={value.width || ""}
        thousandSeparator=" "
        onValueChange={(v) => update({ width: v.floatValue })}
      />
      ...
      <NumericFormat
        ...
        thousandSeparator=" "
        onValueChange={(v) => update({ finished: v.floatValue })}
      />
