...
const rowClass =
  "grid grid-cols-[30px_150px_150px_150px_200px_150px_150px_100px_minmax(200px,1fr)_150px_50px] w-full gap-[10px] p-lg text-primary border-b border-primary items-center min-w-fit w-full";

const headerClass =
  "grid grid-cols-[30px_150px_150px_150px_200px_150px_150px_100px_minmax(200px,1fr)_150px_50px] w-full gap-[10px] p-lg bg-secondary text-primary rounded-t-[8px] border-b border-primary min-w-fit w-full";
...
const getStretching = (length?: number, gazapalLength?: number | string) => {
  const base = Number(gazapalLength || 0);
  if (!base) return 0;
  return Math.floor((Number(length || 0) * 100) / base - 100);
};
...
export const Ram = () => {
  ...
  const [ram, setRam] = useState<Data[]>([]);
  ...
  const refresh = () =>
    axios(`${baseUrl}/printing/ram`)
      .then((res) => setRam(res.data.ram))
  ...
  const groupData = () => {
    ...
    axios(`${baseUrl}/printing/ram/group`, {
      method: "GET",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const selectedGazapal = gazapal?.find((g) => g._id === adding.gazapalId);
    ...
    axios(`${baseUrl}/printing/ram`, {
      ...
      data: {
        ...adding,
        stretching: getStretching(adding.length, selectedGazapal?.length),
      },
    })
      .then((res) => {
  ...
  const handleEdit = (e: React.FormEvent) => {
    ...
    const selectedGazapal = gazapal?.find((g) => g._id === editing.gazapalId);

    axios(`${baseUrl}/printing/ram/${editing._id}`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
      data: {
        ...editing,
        stretching: getStretching(editing.length, selectedGazapal?.length),
      },
    })
      .then((res) => {
        toast.success(res.data.message || res.data.msg);
  ...
  const exportExcel = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", "Ram.xlsx");
  ...
  const deleteOne = (item: Data) => {
    ...
    axios(`${baseUrl}/printing/ram/${item._id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
...
const RamRow = ({
  ...
  const baseLength = readOnly ? value.gazapal?.length : selectedGazapal?.length;
  ...
  return (
    <div className={rowClass}>
      ...
      <NumericFormat
        value={
          readOnly
            ? Math.floor(value.stretching || 0)
            : getStretching(value.length, baseLength)
        }
        displayType="text"
        thousandSeparator=" "
        suffix=" %"
